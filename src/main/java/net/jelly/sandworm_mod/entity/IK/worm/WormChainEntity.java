...
public class WormChainEntity extends KinematicChainEntity {
    ...
    // Active ripple events (server-side tick)
    private final List<WormRippleWorldEvent> activeRipples = new ArrayList<>();
    ...
    private void initWorm() {
        if (segmentCount == 0) {
            for (int i = 0; i < 10; i++)
                addWormSegment(0.35f * 5, new Vec3(1, 0, 0), new Vec3(7.5 * ((i + 2) / 11f), 7.5 * ((i + 2) / 11f), 5));
            for (int i = 0; i < 80; i++)
                addWormSegment(0.35f * 5, new Vec3(1, 0, 0), new Vec3(7.5, 7.5, 5));
    ...
    private void wormAIBehavior() {
        if (thumperTarget == null && (aggroTargetEntity == null || aggroTargetEntity.isRemoved()
                || aggroTargetEntity.isDeadOrDying() || !isDesertBiome(aggroTargetEntity))) {
            retarget(30);
    ...
    public void tick() {
        if (!this.level().isClientSide()) {
            ...
            if (!segments.isEmpty()) {
                if (head == null) {
                    if (segments.get(segmentCount - 1).getClass() == WormHeadSegment.class)
                        head = (WormHeadSegment) segments.get(segmentCount - 1);
                }
    ...
    private void retarget(float yRange) {
        ...
        for (Player entity : nearbyPlayers) {
            if (!isDesertBiome(entity) || this.level().getBrightness(LightLayer.SKY, entity.blockPosition()) <= 0
                    || entity.isSpectator() || entity.isCreative()) continue;
            ...
            if (distanceSq >= 400 && deltaY <= yRange) {
                if (distanceSq < closestDistanceSq) { closestDistanceSq = distanceSq; closestEntity = entity; }
            } else if (deltaY <= yRange) {
                if (distanceSq > fartestTooCloseDistanceSq) { fartestTooCloseDistanceSq = distanceSq; farthestTooCloseEntity = entity; }
            }
        }
    ...
    private boolean predictBreach(Level level, WormSegment head) {
        ...
        return (!level.getBlockState(futureBPos).isSuffocating(level, futureBPos)
                && !level.getBlockState(futureBPos).is(Blocks.CAVE_AIR));
    }

    private void addWormSegment(float length, Vec3 dirVec, Vec3 scale) {
        if (!this.level().isClientSide()) {
            WormSegment chainSegment;
            if (segments.isEmpty()) {
                chainSegment = new WormSegment(ModEntities.WORM_SEGMENT.get(), level());
                chainSegment.moveTo(position());
            } else {
                chainSegment = new WormSegment(ModEntities.WORM_SEGMENT.get(), level());
                chainSegment.moveTo(segments.get(segments.size() - 1).position());
            }
    ...
    public void blastHit() {
        ...
        if (explodedTimes >= CommonConfigs.HEALTH.get()) {
            ItemEntity toothItem = new ItemEntity(this.level(), head.getX(), head.getY(), head.getZ(),
                    new ItemStack(ModItems.WORM_TOOTH.get(), 1));
    ...
    private void smokeParticles(Vec3 pos) {
        WormBreachWorldEvent breachEvent = new WormBreachWorldEvent().setPosition(pos);
        if (this.level() instanceof ServerLevel serverLevel) {
            breachEvent.spawnParticles(serverLevel);
        }
    }
    ...
    private Vec3 getTargetedObjectPos() {
        if (thumperTarget != null) {
            BlockEntity thumper = this.level().getExistingBlockEntity(
                    BlockPos.containing(thumperTarget.x, thumperTarget.y, thumperTarget.z));
