...
public class WormSignHandler {
    ...
    private static int getFeatherFallingLevel(Player player) {
        try {
            ...
            return EnchantmentHelper.getItemEnchantmentLevel(featherFalling, player.getItemBySlot(EquipmentSlot.FEET));
        } catch (Exception e) {
            return 0;
        }
    }
    ...
    public static void tickWS(TickEvent.PlayerTickEvent event) {
        ...
        if (!isDesertBiome(player.getServer().getLevel(player.level().dimension()), player.blockPosition())
            ...
            player.getCapability(WormSignProvider.WS).ifPresent(ws -> decrementWormSign(1, ws));
            return;
        }
        ...
        int softBoots = getFeatherFallingLevel(player);
        player.getCapability(WormSignProvider.WS).ifPresent(ws -> {
